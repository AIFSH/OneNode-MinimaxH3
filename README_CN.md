...
参数区的 **Sigma Refiner** 滑块（0–15 额外步数，默认 1，0 = 关闭）对采样调度的低噪尾部做精修：
保留调度器高噪头部不动，把尾部重采样成更长、更密、更平滑的曲线，让模型在细节收尾阶段多走几步，
消除高速运动边缘的像素颗粒与闪烁。起始/结束阈值与分布曲线使用默认值（起始 0.7、结束 0.0、cosine）。
该节点已原生内置（`H3OneSigmaRefiner`），无需安装额外节点包；算法致谢
[ComfyUI-YCNodes-MiniMax-H3](https://github.com/yichengup/ComfyUI-YCNodes-MiniMax-H3)。

### 二次采样（双采）

参数区的 **Second Pass** 开关启用两段式潜空间精修，参数全自动、无额外控件：
第一遍按正常的全 denoise 调度渲染整段，第二遍把同一份打包的视频+音频 latent
以较低的 `denoise`（自动默认：10 步、denoise 0.4）再送进第二个采样器，
让模型在已有内容上重绘细节而不是从头生成。这是 LTX Stage 1→2 与
[Muse-MiniMax-H3-Refine](https://github.com/muse-collective-26/Muse-MiniMax-H3-Refine)
高清精修所用的经典双采手法，全程在潜空间完成——无需 VAE 往返，也无需额外节点包。
所有模式（含 Chain 的每个片段）均生效。默认关闭，因为开启后采样时间约翻倍。
同时开启 **Latent 放大** 时，T2V / R2V 切换为 RunningHub 的「一采-放大-二采」拆分调度：
调度自动对半拆分——第一遍在基础分辨率跑高 sigma 头部，视频 latent 放大（音频不动），
第二遍在放大分辨率跑低 sigma 尾部（`第一遍 → 放大 → 第二遍 → 解码`）。
原始调度先对半拆分，**Sigma 精修接到第一遍分支**，加步全部在基础分辨率完成；
放大的是第一遍的**干净估计**（`denoised_output`，3D 放大、fp16），采样器强制 `euler`——
若把仍带噪声的中间态送进放大模型（其训练数据是干净 latent），会输出劣化中间结果导致花屏。
R2V 在此模式下不插入第 0 帧身份锚点，与参考工作流一致。
**Chain 模式开启放大时**：同样采用拆分调度——所有片段先在基础分辨率跑高 sigma 头部
（Motion Context 连续性沿用干净的基础 latent），受门控的最终阶段再逐片段放大并跑
低 sigma 尾部（放大节点带 `trigger` 输入，接到最后一片段的 save 输出，确保整个链的
一采全部结束后才开始二采）；二采使用**基础 conditioning 引导器**（不含 MotionContext
关键帧），因为 H3 关键帧行数随画布缩放、放大后无法再采样。
基于关键帧的模式（I2V / Keyframes）
保持输出侧放大（`第一遍 → 第二遍 → 放大 → 解码`）：H3 关键帧行数随目标画布缩放，
而关键帧 latent 仍是基础分辨率编码，放大后无法再采样。
